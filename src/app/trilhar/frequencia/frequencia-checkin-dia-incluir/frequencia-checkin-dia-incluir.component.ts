...
import { ehAniversarioNaData, retornaIdadeFormatadaAnoMesDia } from '../../../shared/funcoes-comuns/utils';
...
export class FrequenciaCheckinDiaIncluirComponent extends BaseListComponent implements OnInit, OnDestroy {
  ...
  override ngOnInit() {
    ...
    this.formularioPesquisar.get('data')?.setValue(new Date());
  ...
  onDataSelecionada(dataSelecionada: DataOutPut): void {
    ...
    this.formularioPesquisar.get('data')?.setValue(this.dataSelecionada);
    this.formularioPesquisar.get('data')?.markAsDirty();
    // this.carregarTurmasAgrupadasPorData(dataSelecionada.data.toISOString().split('T')[0]);
  }

  onBuscarFrequencias() {
    if (this.formularioPesquisar.invalid) {
      this.formularioPesquisar.markAllAsTouched();
      this.mensagemService.showInfo('Por favor, preencha todos os campos obrigatórios.');
      return;
    }
    const data = this.formularioPesquisar.get('data')?.value;
    if (!data) {
      this.mensagemService.showInfo('Data inválida. Por favor, selecione uma data válida.');
      return;
    }
